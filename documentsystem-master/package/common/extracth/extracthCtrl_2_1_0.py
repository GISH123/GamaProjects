...
class ExtracthCtrl:
    ...
    def MakeWorldDataDetail (self, makeTime, hiveCtrl, databaseInfo, printError=False):
        ...
        while exLayer <= 100:
            ...
            for tableIndex, tableRow in exLayerTableDataFrame.iterrows():
                ...
                try:
                    try:
                        ...
                        createStr = createStr.replace("[:FilePath]", databaseInfo["filePathInit"]).replace("[:tableName]", tableRow["tablename"])
                        print(createStr)
                ...
                for worldName in worldNameArr:
                    ...
                    if tableRow["type"] == "grammar" and (tableRow["parenttablename"] == "onliontime" or tableRow["parenttablename"] == "onliontimebyaccount"):
                        partitionedSQLParentInit = partitionedSQLParentInit.replace("dt='[:DateNoLine]'", "dt in (DATE_FORMAT(DATE_ADD('[:DateLine]',0),'yyyyMMdd'),DATE_FORMAT(DATE_ADD('[:DateLine]',1),'yyyyMMdd'))")
                    else:
                        partitionedSQLParentInit = partitionedSQLParentInit.replace("dt='[:DateNoLine]'", "dt = DATE_FORMAT(DATE_ADD('[:DateLine]',0),'yyyyMMdd')")

                    if tableRow["type"] in ["all","grammar"] and pandas.isnull(tableRow["dtbeforeday"]) == False:
                        partitionedSQLOriInit = partitionedSQLInit.replace("dt='[:DateNoLine]'", "dt = DATE_FORMAT(DATE_ADD('[:DateLine]',[:DTBeforeDay]),'yyyyMMdd')")
                    else :
            ...
            self.runsql(hiveCtrl, taskCreateSQLStrArr, printError)
            self.runsql(hiveCtrl, taskSQLStrArr, printError)
    ...
    def getInsertGrammarDataSQL(self):
        return """
            ...
            INSERT OVERWRITE TABLE [:GameName]_[:HiveDB].[:DBName]_[:TableName] PARTITION([:PartitionedAlter]) 
    ...
    def makeExecuteCSV (self, gamename, dbnname, startdate, enddate, tableDF, tableCheck, noFilterNameArray):
        ...
        objectDF = objectDF.ix[:, ["type", "layers", "gamename", "startdate", "enddate"
                                      ...
                                      , "logstartdate", "logenddate", "dtbeforeday", "dtafterday", "logfilename"
                                      , "log", "sql", "memo", "memo2"]]
