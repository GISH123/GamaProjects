...
from maple.LoginTag.modeling.old.RawData_LoginTag_OLD import RawData_LoginTag_OLD

class RawData_LoginTag(RawData_LoginTag_OLD) :

    @classmethod
    def MakeRawData_LoginTag_R0_1_1(self, makeInfo):
        orderSQL1 = """
            WITH BASIC_DATA as (
                SELECT 
                    ...
                    , ROUND(SUM(CASE WHEN AAA.weekday IN (0,6) THEN AAA.d ELSE 0 END ) / 2,5) AS d_holiday
                    , ROUND(SUM(CASE WHEN AAA.weekday IN (0,6) THEN AAA.n ELSE 0 END ) / 2,5) AS n_holiday
                    , ROUND(SUM(CASE WHEN AAA.weekday NOT IN (0,6) THEN AAA.m ELSE 0 END ) / 5,5) AS m_weekday
                    , ROUND(SUM(CASE WHEN AAA.weekday NOT IN (0,6) THEN AAA.d ELSE 0 END ) / 5,5) AS d_weekday
                    , ROUND(SUM(CASE WHEN AAA.weekday NOT IN (0,6) THEN AAA.n ELSE 0 END ) / 5,5) AS n_weekday
                FROM (
                    SELECT 
                        ...
                        , SUM(CASE 
                            ...
                            ELSE 0 
                          END) /28800 / ((datediff('[:DateLine]',DATE_FORMAT(DATE_ADD('[:DateLine]',-6),'yyyy-MM-dd'))+1)/7) AS m
                        , SUM(CASE 
                            ...
                            ELSE 0 
                          END) /28800 / ((datediff('[:DateLine]',DATE_FORMAT(DATE_ADD('[:DateLine]',-6),'yyyy-MM-dd'))+1)/7) AS d
                        , SUM(CASE 
                            ...
                            ELSE 0 
                          END) /28800 / ((datediff('[:DateLine]',DATE_FORMAT(DATE_ADD('[:DateLine]',-6),'yyyy-MM-dd'))+1)/7) AS n
                    FROM gtwpd.modelextract_modelextract AA
                    WHERE 1 = 1 
                        AND AA.game = 'maple'
                        AND AA.tablenumber IN ('1132','1133')
                        AND AA.dt >= DATE_FORMAT(DATE_ADD('[:DateLine]',-6),'yyyyMMdd')
            ...
            SELECT 
                AA.serveraccount as commondata_1
                , null as commondata_2
                , null as commondata_3
                , null as commondata_4
                , AA.pointaccount as commondata_5
                , AA.bfappopenID as commondata_6
                ...
                , AA.m_holiday as UniqueFloat_001
                , AA.d_holiday AS UniqueFloat_002 
                , AA.n_holiday AS UniqueFloat_003 
                , AA.m_weekday AS UniqueFloat_004
                , AA.d_weekday AS UniqueFloat_005
                , AA.n_weekday AS UniqueFloat_006 
    ...
    @classmethod
    def MakeRawData_LoginTag_R0_1_2(self, makeInfo):
        orderSQL1 = """
            WITH BASIC_DATA as (
                SELECT 
                    AAA.commondata_1 as serveraccount
