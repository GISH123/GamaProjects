...
class PostgresCtrl:
    ...
    def insertData(self, tableFullName=None, insertTableInfoDF=None , insertDataDF=None,insertMaxCount=200):
        ...
        for dataIndex, dataRow in insertDataDF.iterrows():
            ...
            for infoIndex, infoRow in insertTableInfoDF.iterrows():
                ...
                if valuesStr != "":
                    valueStr = ","
                if (insertType == "String" or insertType == "Date" or insertType == "Time") and (dataRow[columnName] != None or not pandas.isna(dataRow[columnName])):

                    valueStr = valueStr + "'{}'"
                else:
                    valueStr = valueStr + "{}"
                if pandas.isna(dataRow[columnName]) or dataRow[columnName] == None :
                    dataValueStr = "null"
                else:
    ...
    def updateData(self, tableFullName=None, updateTableInfoDF=None, updateData = None):
        conn = psycopg2.connect(database=self.__database, user=self.__user, password=self.__password, host=self.__host, port=self.__port)
        cur = conn.cursor()
        schemaName = tableFullName.split(".")[0] if tableFullName.find(".") >= 0 else self.__schema
        tableName = tableFullName.split(".")[1] if tableFullName.find(".") >= 0 else tableFullName
        ...
        for infoIndex, infoRow in updateTableInfoDF.iterrows():
            ...
            elif (insertType == "String" or insertType == "Date" or insertType == "Time") and updateData[columnName] != None:
                columnsValuesStr = columnsValuesStr + "{} = '{}'".format(infoRow["columnname"],updateData[columnName])
            elif updateData[columnName] != None :
                columnsValuesStr = columnsValuesStr + "{} = {}".format(infoRow["columnname"],updateData[columnName])
            else :
                columnsValuesStr = columnsValuesStr + "{} = null".format(infoRow["columnname"])

        whereStr = "{}id = {}".format(tableName,updateData["{}id".format(tableName)])
        ...
        print(updateCodeStr)
        cur.execute(updateCodeStr)
        print("已更新 {} 1 筆資料".format(tableName))
        cur.execute("commit;")
        conn.close

    # 將Dataframe，使用IO方式，直接塞入資料庫指定Table
    def writeDFToTable(self, df, table_name, if_exists="fail"):
        db_engine = create_engine("postgresql://"+self.__user+":"+self.__password+"@"+self.__host+":"+str(self.__port)+"/"+self.__database)
        string_data_io = io.StringIO()
        df.to_csv(string_data_io, sep="|", index=False)
        pd_sql_engine = pd.io.sql.pandasSQL_builder(db_engine)
        table = pd.io.sql.SQLTable(table_name, pd_sql_engine
                                   , frame=df, index=False, if_exists=if_exists, schema=self.__schema)
        table.create()
