...
class PostgresCtrl:
    ...
    def checkTable(self, table_name):
        ...
        cur = conn.cursor()
        cur.execute("SELECT * FROM information_schema.tables where table_schema = '" + self.__schema + "' AND table_name = '" + table_name + "'")
        is_exist = False
        if cur.rowcount > 0:
            is_exist = True
        else:
    ...
    def writeDFToTable(self, df, table_name, if_exists="fail",float_format=None):
        ...
        table = pd.io.sql.SQLTable(table_name, pd_sql_engine , frame=df, index=False, if_exists=if_exists, schema=self.__schema)
        table.create()
