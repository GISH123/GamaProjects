...
from sql.gamania.modelextract.ReportInfo import ReportInfo
...
def main(parametersData = {}):
    ...
    gameIdArr = [] if gameIdArr == [] else gameIdArr
    gameIdCodeArr = reportInfo.gameIdCodeArr if gameIdArr == [] else []
    if gameIdCodeArr == []:
        for gameId in gameIdArr:
            for gameIdCode in reportInfo.gameIdCodeArr:
                if gameIdCode[1] == gameId:
                    gameIdCodeArr.append(gameIdCode)
    sourceTable = ["beanfundb_history_play", "beanfundb_history_deposit"]
    ...
    for makeTime in makeTimeArr:
        ...
        makeInfo = {
            ...
            , "layerArr": layerArr
            , "gameIdCodeArr": gameIdCodeArr
            , "HistoryPlay": sourceTable[0]
            , "HistoryDeposit": sourceTable[1]
...
def makeModelExtract(makeInfo):
    ...
    HistoryPlay = makeInfo["HistoryPlay"]
    HistoryDeposit = makeInfo["HistoryDeposit"]
