...
def MakeInitDataMain(makeTime):
        if makeTime <= datetime.datetime.strptime("2020-10-31", "%Y-%m-%d"):
            databaseInfo = {
                "gameName": "mabi"
                , "dbName": "init"
                , "tableName": "loginlogoutlist"
                , "insertSqlInit": getInsertDataSQLByLoginLogoutOld()
                , "partitionedInit": "dt={}"
                , "gameServer": [""]
            }
...
def getInsertDataSQLByLoginLogout():
        ...
        WITH BasicData AS (
                SELECT DISTINCT
                     ...
                     CASE WHEN channel LIKE '%GS_A%' THEN '01'
                     	  WHEN channel LIKE '%GS_B%' THEN '02'
                     	  WHEN channel LIKE '%GS_C%' THEN '03' END AS world,
	                 CASE WHEN logdetail LIKE "%entered game%" THEN SPLIT(TRANSLATE(logdetail, ' ',''), "\\(|\\)|\\[|\\]|\\@|\\/|\\,|\\'")[8]
       				 ...
       				 CASE WHEN logdetail LIKE "%entered game%" THEN 'login'
       					  ...
       					  END AS logtype
                FROM mabi_all.mabi_log_gamelog AA
                WHERE 1 = 1 
                ...
                AND (
                        ...
                        (
                            '[:DateNoLine]' < '20210106'
                            AND '[:DateNoLine]' >= '20201101'
                            AND AA.date >= '20210101'
                            AND AA.date <= '20210112'
                        )
                )
                AND channel not like '%House%'
            )
...
def getInsertDataSQLByLoginLogoutOld():
    return r"""
        WITH BasicData AS (
                SELECT DISTINCT
