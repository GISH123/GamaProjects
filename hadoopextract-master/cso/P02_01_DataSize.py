...
def getHadoopDataDF (makeZeroTime,gameName,dbName):
    hadoopStr = "hadoop dfs -du /user/hive/warehouse/{}.db/ALL/{}/dt={}/*".format(gameName,('*' if dbName == 'DB' else dbName),makeZeroTime.strftime("%Y%m%d"))
    #print(hadoopStr)
    ...
    for retrunStr in retrunStrArr:
        ...
        if retrunStr == "DEPRECATED: Use of this script to execute hdfs command is deprecated." :
            ...
            continue
        elif retrunStr.find("New Text Document") >= 0:
            continue
        else:
            def not_empty(s):
    ...
    mainDF = mainDF.astype({"datasize": 'int64'})
