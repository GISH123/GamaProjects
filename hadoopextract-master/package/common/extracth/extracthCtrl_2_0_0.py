...
class ExtracthCtrl:
    ...
    def MakeWorldDataDetail (self,makeTime,hiveCtrl,databaseInfo,printError=False) :
        ...
        extractTableType = databaseInfo["extractTableType"] if "extractTableType" in databaseInfo.keys() else "File"

        if  extractTableType == "File" :
            tableDataFrame = pandas.read_csv(databaseInfo["csvfile"], encoding="utf_8_sig")
        elif extractTableType == "DB" :
            ...
            searchSQL = """
                select * 
                from extractinfo.extractmakeinfo AA
                where 1 = 1 
                    and AA.gamename = '[:GameName]'
                    and AA.dbname = '[:DBName]'
            """.replace("[:GameName]",gameName).replace("[:DBName]",dbName)
            tableDataFrame = postgresCtrl.searchSQL(searchSQL)
            tableDataFrame = tableDataFrame.rename(columns={'datatype':'type'})
