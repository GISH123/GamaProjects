...
def main(parametersData = {}):
    ...
    while makeTime <= endDateTime:
        makeIPMain(makeTime,world,fileDate = fileDate,maintainDay = maintainDay)
        #makeIPAccMain(makeTime, fileDate =fileDate,prepareDays = maintainDay)
        makeTime = makeTime + dt.timedelta(days=1)
    t2 = dt.datetime.now()
    print(f'cumulative time consumed :{t2 - nowTime}')

def makeIPMain(makeTime,gwList,fileDate,maintainDay = 3):
    d = makeTime
    ...
    # step 0: 創資料表
    sqlReplaceArr =[['[:Date0]', d_str],['[:fileDate]', fileDate]]
    hiveStrArr = SqlTool().loadfile('file/maple/ip_SQL/00_createIPLogDataTemp_retore.sql').makeSQLStrs(
    ...
    for gw in gwList:
        ...
        for i in range(runTables.shape[0]):
            ...
            #print(hiveStr[0])
            if runTables['sqlName'].iloc[i] == "01_iplog_select.sql" :
...
def makeIPAccMain(makeTime,fileDate,prepareDays = 3):
    ...
    # step 0: 創資料表
    sqlReplaceArr = [['[:Date0]', d_str],['[:fileDate]', fileDate]]
    hiveStrArr = SqlTool().loadfile('file/maple/ip_SQL/20_createLogIPLogDataTemp_restore.sql').makeSQLStrs(sqlReplaceArr=sqlReplaceArr).makeSQLArr().getSQLArr()

    for hiveStr in hiveStrArr:
        print(hiveStr)
        hiveCtrl.executeSQL_TCByCount(hiveStr,3)
...
if __name__ == "__main__":
    main(parametersData = {'startdate': ['2021-07-01'], 'enddate': ['2021-07-04'], 'fileDate':['20210801'] , 'world':['45']})