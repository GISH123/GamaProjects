...
def main(parametersData={}):
    ...
    for makeTime in makeTimeArr:
        if makeTime <= datetime.datetime(2020, 7, 31, 0, 0, 0, 0) :
            continue
...
def MakeRestoreFile(makeZeroTime, gameName, dbName):
    ...
    for dataCheckIndex, dataCheckRow in dataCheckDF.iterrows():
        ...
        shfileMake = open("file/tdn/file_restore/{}".format(fileName), "w", encoding="utf-8")
...
select 
 	...
 	, AA.gamename 
	, AA.dbname
	, '/user/hive/warehouse/tdn.db/Restore/' || AA.dbname || '/dt=' || to_char(AA.datatime,'yyyymmdd') as initpath
...
select 
	...
	, case 
		when BB.datatype = 'log_file'
			then '/user/hive/warehouse/tdn.db/' || BB.orihivedb || '/' || BB.oridbname || '/dt=' 
		...
		when BB.datatype = 'copy_new'
			then '/user/hive/warehouse/tdn.db/' || BB.orihivedb || '/' || BB.oridbname || '/dt=' || to_char(AA.datatime + INTERVAL '1 Day','yyyymmdd') || '/' || BB.oritablename
	  end as oripath
	, '/user/hive/warehouse/tdn.db/'|| BB.hivedb || '/' || BB.dbname || '/dt=' ||to_char(AA.datatime,'yyyymmdd') || '/' ||BB.tablename as restorepath	 
	, '/user/hive/warehouse/tdn.db/'|| 'ALL' || '/' || BB.dbname || '/dt=' ||to_char(AA.datatime,'yyyymmdd') || '/' || BB.tablename as allpath	 
from datacheck.datacheckdetail AA
inner join restoreinfo.restoreinfo BB on 1 = 1 
	and BB.gamename = AA.gamename 
	and BB.dbname = AA.dbname
