...
def MakeGamePartitionMain(makeTime,gameName,dbName,runALL=True,runRestore=True):
    ...
    if dbName == 'DB' or dbName == 'dnworld' :
    ...
    if dbName == 'DB' or dbName == 'dnmembership':
        MakeGamePartitionDetail(makeTime,databaseInfo)
    ...
    if dbName == 'DB' or dbName == 'dnstaging':
        MakeGamePartitionDetail(makeTime, databaseInfo)
...
def MakeGamePartitionDetail(makeTime,databaseInfo):
    ...
    taskSQLStrArr = []
    for gameWorldArr in gameWorldArrMap:
        hadoopStr = "hadoop dfs -du -s /user/hive/warehouse/tdn.db/Restore/{}/dt={}/*".format(gameWorldArr[1], makeTime.strftime("%Y%m%d"))
